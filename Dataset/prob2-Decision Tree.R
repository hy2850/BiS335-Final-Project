...
random <- shuffle(nrow(data)) # Used in k-fold cross validation (random index selector)
...
while(1==1){
  ...
  for (j in candi){
    ...
    for (i in 1:5){
      ...
      training <- new_data[-indices,]
      
      ####################### Change this to apply to other models ##########################
      model <- tree(surv_ind ~. -sample_id, data = training, subset = -indices)
      
      #if(inherits(model, "singlenode")) stop("cannot plot singlenode tree")
      #if(!inherits(model, "tree")) stop("not legitimate tree")
      # if(!inherits(model, "singlenode")){
      #   model.prune <- cv.tree(model, FUN = prune.misclass)
      #   model.best <- prune.misclass(model, best = model.prune$size[tail(which(model.prune$dev == min(model.prune$dev)),1)])
      #   if(!inherits(model.best, "singlenode"))
      #     pred.test <- predict(model.best, validation, type = "class")
      #   else
      #     pred.test <- predict(model, validation, type = "class")
      #   
      # }else
      #   pred.test <- predict(model, validation, type = "class")
      ...
      acc.test[i] <- mean(validation$surv_ind == pred.test)
    }
  ...
  if (temp[length(temp)] < max(rate)){ # If newly selected feature increases the max accuracy of the model, select that feature
    print("Forward selection max accuracy : "); print(max(rate))
  ...
  if (length(selected) <= 3) {next} # skip if selected has only one selected predicators (2 are sample_id and surv_ind)
  ...
  for (j in 3:length(selected[-1])){ # Excluded recently added feature, because removing it right after it is being added is pointless
    acc.test <- c()
  ...
  if (temp[length(temp)] < max(rate2)){ # If deleting certain feature increases the max accuracy of the model, delete that feature completely
    print("Backward selection max accuracy : "); print(max(rate2))    
  ...
  print("Final selection : "); print(selected)
}